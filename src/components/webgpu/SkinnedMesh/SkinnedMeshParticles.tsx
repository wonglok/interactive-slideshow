...
export let setupSkinMesh = async ({ skinnedMesh, mounter, renderer, onLoop }: any) => {
    ...
    const createBuffer = ({ itemSize = 3, type = 'vec3' }) => {
        let attr = new THREE.StorageInstancedBufferAttribute(particleCount, itemSize)
    ...
    const boneMatricesNode = {
        node: buffer(skinnedMesh.skeleton.boneMatrices, 'mat4', skinnedMesh.skeleton.bones.length),
    }
...

    {
        for (let i = 0; i < particleCount; i++) {
            ...
            let x = geo.attributes.position.getX(yo) + (Math.random() * 2 - 1.0) * 2.0 * boundingBoxSize.x * 0.01
            let y = geo.attributes.position.getY(yo) + (Math.random() * 2 - 1.0) * 2.0 * boundingBoxSize.y * 0.01
            let z = geo.attributes.position.getZ(yo) + (Math.random() * 2 - 1.0) * 2.0 * boundingBoxSize.z * 0.01
    ...
    const finalColor = mix(color('#40ffdc'), color('#d5fbff'), pow(velAttr.y.abs().mul(50), 1.5))
    ...
    // const example = new THREE.SphereGeometry(1, 4, 4)

    instGeometry.setAttribute('position', new THREE.BufferAttribute(example.attributes.position.array, 3))
    instGeometry.setAttribute('normal', new THREE.BufferAttribute(example.attributes.normal.array, 3))
    instGeometry.setAttribute('uv', new THREE.BufferAttribute(example.attributes.uv.array, 2))
    instGeometry.setIndex(example.index)

    const positionArray = new Float32Array(particleCount * 3)
    instGeometry.setAttribute('instancePos', new THREE.InstancedBufferAttribute(positionArray, 3))

    const lifeValue = lifeBuffer.node.element(instanceIndex)

    const material = new THREE.MeshPhysicalNodeMaterial()
    material.colorNode = color('#000000')
    material.emissiveNode = finalColor.mul(lifeValue.y).mul(2.5)
    ...
    material.vertexNode = Fn(() => {
        ...
        const life = lifeBuffer.node.element(instanceIndex)
        const unitScale = pow(velocity.x.abs().add(velocity.y.abs()).add(velocity.z.abs()), 2.1).mul(20).mul(life.y)
