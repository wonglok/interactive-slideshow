...
import { float, PI, cos, sin, mat3, vec3, Fn, mul, add, abs, pow, sub } from 'three/tsl'
...
const noise: any = Fn(([p_immutable]: any) => {
    const p = vec3(p_immutable).toVar()

    return cos(p.x).mul(sin(p.y).mul(cos(p.z)))
})

const fbm4: any = Fn(([p_immutable]: any) => {
    const p = vec3(p_immutable).toVar()
...
const fbm6: any = Fn(([p_immutable]: any) => {
    const p = vec3(p_immutable).toVar()
    const f = float(0.0).toVar()
...
const pattern: any = Fn(([p_immutable, time_immutable]: any) => {
    const time = float(time_immutable).toVar()
    const p = vec3(p_immutable).toVar()
    const vout = float(fbm4(p.add(time.add(fbm6(p.add(fbm4(p.add(time)))))))).toVar()
