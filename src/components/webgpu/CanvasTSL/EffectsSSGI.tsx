...
import { Clock, FloatType, HalfFloatType, WebGLRenderer } from 'three'
import { useStoreOfApp, type AppInitStateType } from '../CanvasEditor/AppContext'
...
export function EffectsSSGI() {
    ...
    let lighting = useDynamicStore((r: any) => r.lighting) as SunlightObject | null
    ...
    useEffect(() => {
        ...
        const compositePass = vec4(add(scenePassColor.rgb.mul(ao), scenePassDiffuse.rgb.mul(gi)), alpha)
        ...
        const applyRenderSettings = (now: AppInitStateType) => {
            if (now.render === 'ssgi') {
                ...
                postProcessing.outputNode = vec4(add(scenePassColor, bloomPass).rgb, alpha)
                postProcessing.needsUpdate = true
